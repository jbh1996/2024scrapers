...
for number in range (1, 10):
    url_list.append( f'https://esersws.mt.gov/ResultsAjax.svc/GetMapData?type=BQ&category=PREC&raceID=450000000004780&county=0{number}&party=0')
for number in range (10, 57):
    url_list.append(f'https://esersws.mt.gov/ResultsAjax.svc/GetMapData?type=BQ&category=PREC&raceID=450000000004780&county={number}&party=0')
...
outfile_temp = open(f'2024CI127Results{datetime}.csv', 'w')
outfile_temp.write(csv_string)
df = pd.read_csv(f'2024CI127Results{datetime}.csv')
# df.drop_duplicates(inplace=True)
df.to_csv(f'2024CI127Results{datetime}.csv')
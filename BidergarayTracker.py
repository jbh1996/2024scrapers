...
for number in range (1, 10):
    url_list.append( f'https://esersws.mt.gov/ResultsAjax.svc/GetMapData?type=SCJ&category=PREC&raceID=450000000001615&county=0{number}&party=0')
for number in range (10, 57):
    url_list.append(f'https://esersws.mt.gov/ResultsAjax.svc/GetMapData?type=SCJ&category=PREC&raceID=450000000001615&county={number}&party=0')
...
outfile_temp = open(f'BIDEGARAYResults{datetime}.csv', 'w')
outfile_temp.write(csv_string)
df = pd.read_csv(f'BIDEGARAYResults{datetime}.csv')
# df.drop_duplicates(inplace=True)
df.to_csv(f'BIDEGARAYResults{datetime}.csv')
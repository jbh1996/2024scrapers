...
for number in range (1, 10):
    url_list.append( f'https://esersws.mt.gov/ResultsAjax.svc/GetMapData?type=SENATE&category=PREC&county=0{number}&party=0')
for number in range (10, 57):
    url_list.append( f'https://esersws.mt.gov/ResultsAjax.svc/GetMapData?type=SENATE&category=PREC&county={number}&party=0')
...
outfile_temp = open(f'Senate{datetime}.csv', 'w')
outfile_temp.write(csv_string)
df = pd.read_csv(f'Senate{datetime}.csv')
# df.drop_duplicates(inplace=True)
df.to_csv(f'Senate{datetime}.csv')
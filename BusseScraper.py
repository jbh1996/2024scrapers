...
for number in range (1, 10):
    url_list.append( f'https://esersws.mt.gov/ResultsAjax.svc/GetMapData?type=STATE&category=PREC&raceID=450000000001605&county=0{number}&party=0')
for number in range (10, 57):
    url_list.append(f'https://esersws.mt.gov/ResultsAjax.svc/GetMapData?type=STATE&category=PREC&raceID=450000000001605&county={number}&party=0')
...
outfile_temp = open(f'BUSSE{datetime}.csv', 'w')
outfile_temp.write(csv_string)
df = pd.read_csv(f'BUSSE{datetime}.csv')

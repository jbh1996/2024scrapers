...
for number in range (1, 10):
    url_list.append( f'https://esersws.mt.gov/ResultsAjax.svc/GetMapData?type=FED&category=PREC&raceID=450000000001602&county=0{number}&party=0')
for number in range (10, 57):
    url_list.append(f'https://esersws.mt.gov/ResultsAjax.svc/GetMapData?type=FED&category=PREC&raceID=450000000001602&county={number}&party=0')
for number in range(1, 10):
        url_list.append(f'https://esersws.mt.gov/ResultsAjax.svc/GetMapData?type=FED&category=PREC&raceID=450000000001601&county=0{number}&party=0')
for number in range(10, 57):
        url_list.append(
...
outfile_temp = open(f'Tester{datetime}.csv', 'w')
